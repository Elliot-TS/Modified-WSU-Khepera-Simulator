...
public class WorldPanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    protected Graphics2D     wg2;          // WorldPanel's context

   private BufferedImage  tempImage;
  protected Graphics2D     tempg2;
    ...
    */
    ...
    /**
     * This fixes the Lights on saved map bug!!When maps were being saved,the worldObjects 
     * vector was serialized and saved and retrieved when maps were loaded but the lightObjects 
     * vector was not being initialized simultaneously.The sim uses the lightObjects vector to
     * determine light intensity and hence the lights from saved maps did not work.I initialize
     * the lightObjects vector also when we load the worldObjects vector from saved files and so now
     * the light bug is fixed and works fine--Aarti Jan '06
     */
    ...
    protected void setWorldObjects(Vector wo) {
        ...
        Vertex vtx;
        while(e.hasMoreElements()) {
        	vtx = (Vertex) e.nextElement();
            if(vtx.objType == WorldPanel.LIGHT)
            	lightObjects.add(vtx);
        }    	
