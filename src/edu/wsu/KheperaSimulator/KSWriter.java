...
public class KSWriter implements Runnable {
  ...
  public void run() {
    while(record) {
      ...
      float[] data = new float[31]; //was 23 for old writer
      for (int i = 0; i < 8; i++) {
      ...
      //NEW STUFF-LVT STUFF
      data[23] = nFrame.drawManager.rState.getRobotCoordinates().lvtPoints[0].x;
      data[24] = nFrame.drawManager.rState.getRobotCoordinates().lvtPoints[0].y;
      data[25] = nFrame.drawManager.rState.getRobotCoordinates().lvtPoints[1].x;
      data[26] = nFrame.drawManager.rState.getRobotCoordinates().lvtPoints[1].y;
      data[27] = nFrame.drawManager.rState.getRobotCoordinates().lvtPoints[2].x;
      data[28] = nFrame.drawManager.rState.getRobotCoordinates().lvtPoints[2].y;
      data[29] = nFrame.drawManager.rState.getRobotCoordinates().lvtPoints[3].x;
      data[30] = nFrame.drawManager.rState.getRobotCoordinates().lvtPoints[3].y;
      ...
      state.data = data;
      state.lvtData = new float[64];
      //NEW LVT STUFF
      for(int j=0;j<64;j++)
      {
      	state.lvtData[j] = nFrame.drawManager.rState.getLvtValue(j);
      }
      //END NEW LVT STUFF
      state.sensorData = new float[32];
      //NEW SENSOR STUFF
      for(int k=0;k<32;k++)
      {
      	state.sensorData[k] = nFrame.drawManager.rState.getSensorCoord(k);
      }
