...
public class KSReader implements Runnable{
  ...
  private float[] lastLvtValue = null;//needed for rewind
  private float[] lastSensorCoords = null;//needed for rewind
  // END NEW
           ...
           if(state.worldChange) {
             ...
             while(!found) {
               ...
               if(tState.worldChange) {
                 ...
                 lastLvtValue = new float[64];
                 lastLvtValue = tState.lvtData;
                 lastSensorCoords = new float[32];
                 lastSensorCoords = tState.sensorData;
                 found = true;
                 break;
               }
               temp--;
             ...
             if(found) {
               ...
               frame.drawManager.rState.setLvtArray(lastLvtValue);
               frame.drawManager.rState.setSensorCoordArray(lastSensorCoords);
             }
             else {
               frame.worldPanel.setHeldId(0);
               frame.worldPanel.setWorldObjects(v);
               for(int k = 0; k < 64; k++){
             	  frame.drawManager.rState.setLvtValue(state.lvtData[k],k);  
               }
               for(int l = 0; l < 32; l++){
              	  frame.drawManager.rState.setSensorCoord(state.sensorData[l],l);  
                }
             }
           }
           // END NEW
            ...
            //NEW STUFF_LVT STUFF
            frame.drawManager.rState.getRobotCoordinates().setLvtCoordinates(data[23],data[24],0);
            frame.drawManager.rState.getRobotCoordinates().setLvtCoordinates(data[25],data[26],1);
            frame.drawManager.rState.getRobotCoordinates().setLvtCoordinates(data[27],data[28],2);
            frame.drawManager.rState.getRobotCoordinates().setLvtCoordinates(data[29],data[30],3);
            for(int k = 0; k < 64; k++){
          	  frame.drawManager.rState.setLvtValue(state.lvtData[k],k);  
            }
            //END OF LVT STUFF
