  ...
  public void run() {
    ...
    try {
      ...
      while (!(inData = (String)in.readObject()).equals("EOT")) {
        ...
        else if (inData.startsWith("600")) {
        	float lvtdata[] = new float[64]; 
        	lvtdata = (float[])in.readObject();
        	frame.drawManager.rState.setLvtArray(lvtdata);
        }
        else if (inData.startsWith("800")) {
        	float sensorData[] = new float[64]; 
        	sensorData = (float[])in.readObject();
        	frame.drawManager.rState.sensorCoordArray(sensorData);
        }
      } // while
      ...
      kill();
  	  ...
  	  System.out.println("NOT WORKINGGGGGGG");
      kill();
    }
  } // run
