...
public class KSServerWorker extends Thread {
 ...
 /**
  ...
  private synchronized void transmit() throws Exception {
    ...
    float[] data = new float[31]; // you must create a new object each time, read api for ObjectOutputStream
    ...
    data[23] = frame.drawManager.rState.getRobotCoordinates().lvtPoints[0].x;
    data[24] = frame.drawManager.rState.getRobotCoordinates().lvtPoints[0].y;
    data[25] = frame.drawManager.rState.getRobotCoordinates().lvtPoints[1].x;
    data[26] = frame.drawManager.rState.getRobotCoordinates().lvtPoints[1].y;
    data[27] = frame.drawManager.rState.getRobotCoordinates().lvtPoints[2].x;
    data[28] = frame.drawManager.rState.getRobotCoordinates().lvtPoints[2].y;
    data[29] = frame.drawManager.rState.getRobotCoordinates().lvtPoints[3].x;
    data[30] = frame.drawManager.rState.getRobotCoordinates().lvtPoints[3].y;
    ...
    float lvtdata[] = new float[64];
    for(int j=0;j<64;j++)
    {
    	lvtdata[j] = frame.drawManager.rState.getLvtValue(j);
    }
    out.writeObject(lvtdata);
    out.flush();
    
//  send sensor line drawing data
    outData = "800 Sensor";
    out.writeObject(outData);
    out.flush();

    float sensorData[] = new float[32];
    for(int k=0;k<32;k++)
    {
    	sensorData[k] = frame.drawManager.rState.getSensorCoord(k);
    }
