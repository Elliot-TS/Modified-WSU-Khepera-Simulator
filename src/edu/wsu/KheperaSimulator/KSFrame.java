...
public class KSFrame extends JFrame {
  ...
  /**Check Boxes for Lvt and Sensor Display**/
  private JCheckBox rbLvt = new JCheckBox("LVT Display",false);
  private JCheckBox rbSensor = new JCheckBox("Sensor Display",false);
  ...
  protected LvtPanel lvtPanel = new LvtPanel();
  ...
  private void initComponents() {
    ...
    */
    ...
    rbLvt.addItemListener(checkBoxListener);
    rbSensor.addItemListener(checkBoxListener);
    ...
    textArea.append("WSU Khepera Robot Simulator v7.3\n\n");
    ...
    rbLvt.setBackground(Color.white);
    rbSensor.setBackground(Color.white);
    ...
    selectSensorPanel.setLayout(new GridBagLayout());
    GridBagConstraints c = new GridBagConstraints();
    c.fill = GridBagConstraints.HORIZONTAL;
    c.weightx = .5;
    c.gridx = 0;
    c.gridy = 0;
    selectSensorPanel.add(rbDistance,c);
    c.gridx = 1;
    c.gridy = 0;
    selectSensorPanel.add(rbLight,c);
    c.gridx = 2;
    c.gridy = 0;
    selectSensorPanel.add(rbNothing,c);
    c.gridx = 1;
    c.gridy = 1;
    selectSensorPanel.add(rbLvt,c);
    c.weightx = 0.5;
    c.gridx = 2;
    c.gridy = 1;
    selectSensorPanel.add(rbSensor,c);
    ...
    //c.gridx = 0;
    //c.gridy = 0;
    //checkBoxPanel.add(rbLvt,c);
    //c.gridx = 0;
    //c.gridy = 1;
    //checkBoxPanel.add(rbSensor,c);
    //checkBoxPanel.setBackground(Color.white);

    /* Robot State Panel */
    //robotStatePanel.setLayout(new FlowLayout(FlowLayout.CENTER, 35, 0));
    robotStatePanel.setLayout(new BoxLayout(robotStatePanel,BoxLayout.Y_AXIS));
    ...
    //sensorDisplayPanel.add(checkBoxPanel, BorderLayout.WEST);
    sensorDisplayPanel.add(robotStatePanel, BorderLayout.EAST);
    sensorDisplayPanel.add(lvtPanel,BorderLayout.SOUTH);
    ...
    //eastSplitPane.setBottomComponent(lvtPanel);
  ...
  /**
   * MyCheckBoxListener for LVT and sensor check boxes
   * Get the appropriate action and update the appropriate display mode.
   */
  class MyCheckBoxListener implements ItemListener {
    public void itemStateChanged(ItemEvent e) {
      ...
      if (action == ItemEvent.SELECTED){
    	  if(e.getItem().equals(rbLvt))
    	  {
    		  drawManager.setLvtMode(true);
    	  }
    	  if(e.getItem().equals(rbSensor))
    	  {
    		  drawManager.setSensorMode(true);
    	  }
      }
      else if (action == ItemEvent.DESELECTED){
    	  if(e.getItem().equals(rbLvt))
    	  {
    		  drawManager.setLvtMode(false);
    	  }
    	  if(e.getItem().equals(rbSensor))
    	  {
    		  drawManager.setSensorMode(false);	  
    	  }
      }
    }
    } // CheckBoxListener
